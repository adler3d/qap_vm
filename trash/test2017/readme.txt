...
Хранилище скриптов{}или{
  nodejs{
    ...
    история:
    есть nodejs сервер, но ненадёжный(я его буду перезапускать, он будет переезжать)
    скрипты заливаються на него.
    нужен теневой запасной сервер - реплика. на который будут заливаться все скрипты.
    если главный упадёт, то я выкачаю скрипты с реплики.
    если и реплика упадёт, то востановлю из бекапа. с потерей части свежыйх скриптов/изменений.
    если теневой сервер не сделать, то это будет трэшь который стыдно показывать. так невозможно работать.
    
    //готово: нужно сделать теневой сервер. // готово
    
    //готово: нужно чтобы сервер знал свой url // готово
    
    //готово: сделать: публичный сервер сразу после старта выкачивает файлы из теневого сервера. // готово
    
    //готово: теневой сервер должен отправлять /ping к основному серверу, если за 40 сек неприходит /tick от основного сервера.
...
Hosts:{
  ...
  /*
  30days(from 2017.03.23): http://vm-vm.44fs.preview.openshiftapps.com/
