  ...
  t_calc::i_stat::i_visitor
{
  ...
  void Do(t_calc::t_varcall& r){
    ...
    if(!r.params){
      set(ctx.vars[name].num);
      return;
    }
    auto it=ctx.funcs.find(name);
    if(it==ctx.funcs.end())for(int i=0;;i++){
      auto lvl=int(ctxs.size())-1-i;
      if(lvl<0){cerr<<"fatal error! function not found: "<<name<<endl;exit(-1);}
      auto&f=ctxs[lvl].funcs;
      it=f.find(name);
    ...
    if(!fn.body){
      ...
      for(size_t i=0;i<fn.args.size();i++){
        Do(r.params->arr[i].body);
        ctx.vars[fn.args[i]] = last;
      }
      Do(*fn.body);
      //t_value ret = last;
      ctx=std::move(ctxs.back());
      ctxs.pop_back();
      //last = ret;
    }
  }
